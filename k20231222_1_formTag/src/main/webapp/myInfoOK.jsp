...
<body>
	...
	// get 방식도 톰캣 6.0 버전까지는 한글이 깨졌었다. -> 톰캣 7.0 버전부터 깨지지 않는다.
	// get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야한다. ->server.xml
	// Servers 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8"속성을 추가한다.
	// 서버에 추가하기 때문에 프로젝트를 생성할 때마다 설정할 필요는 없고, 이클립스를 다시 세팅할 때
	// 해주면 된다.
	...
	// checkbox는  여러 항목을 선택할 수 있는데 아래와 같이 request.getParameter() 메소드를 이용하면
	// 맨처음에 선택한 값 1개만 넘겨받는다.
	// 데이터가 여러개 넘어오는 경우 request.getParameter() 메소드를 사용하지 않는다.
	//	String hobbies = request.getParameter("hobbies");
	//	out.println(name + "님의 취미는 " + hobbies + " 입니다.<br>");

	// 	checkbox에서 넘어오는 데이터를 받을 때 넘어오는 데이터가 여러개일 수 있기 때문에
	//  request.getParameterValues() 메소드로 받아서 배열에 저장해서 사용된다.
	String[] hobbies = request.getParameterValues("hobbies");
		out.println("님의 취미는 " + hobbies + "입니다.<br>");
		out.println("님의 취미는 " + Arrays.toString(hobbies) + "입니다.<br>");
		out.println("님의 취미는 ");
	try {
	...
	String trip = request.getParameter("trip");
		out.println(name +"님의 " + trip + "에 가고 싶어합니다.<br>");
	...
	try {
		...
		out.println("가고 싶어 하는 곳이 없습니다. <br>");
	}
