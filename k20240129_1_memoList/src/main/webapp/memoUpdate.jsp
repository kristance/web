...
//	수정할 글번호와 수정 후 돌아갈 페이지 번호가 모두 넘어왔나 검사한다.
	if (tempIdx != null && tempIdx.trim().length() != 0 &&
		...
		//	수정할 글번호와 수정 후 돌아갈 페이지 번호가 모두 숫자인가 검사한다.
		try {
			int idx = Integer.parseInt(tempIdx);
			int currentPage = Integer.parseInt(tempCurrentPage);
			
			//	수정할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기위해서 글번호에 해당되는 글을
			//	테이블에서 얻어온다.
...
<form action="updateOK.jsp" method="post">
	<table width="700" border="1" align="center" cellpadding="5" cellspacing="0">
		...
		<tr>
			<th>이름</th>
			<td>
				<%-- <%=rs.getString("name").replace("<", "&nt;").replace(">", "&gt;")%> --%>
		...
		<tr>
			<th>메모</th>
			<td>
				<%-- <%=rs.getString("memo").replace("<", "&nt;").replace(">", "&gt;")%> --%>
	...
	<!-- 수정할 글번호와 수정 후 돌아갈 페이지 번호도 deleteOK.jsp로 넘겨줘야한다. -->
...
<%				
			...
			//	수정가 글이 테이블에 존재하지 않기 때문에 에러메시지를 출력하고
		...
		//	수정가 글번호 또는 돌아갈 페이지 번호가 숫자가 아니므로 에러메시지를 출력하고
		//	memoList4.jsp로 돌려보낸다.
			out.println("<script>");
		...
		//	수정가 글번호 또는 돌아갈 페이지 번호가 넘어오지 않았으므로 에러메시지를 출력하고
		//	memoList4.jsp로 돌려보낸다.
		out.println("<script>");
