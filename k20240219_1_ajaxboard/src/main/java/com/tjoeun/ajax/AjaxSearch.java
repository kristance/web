...
public class AjaxSearch extends HttpServlet {
	...
	protected void actionDo(HttpServletRequest request, HttpServletResponse response)
		...
		// response.getWriter()의 write() 메소드로 AJAX 방식으로 요청한 곳으로 데이터를 리턴한다.
		// write() 메소드의 인수는 반드시 문자열로 지정해야 하며 AJAX로 서블릿을 호출한 곳에서는
		// responseText를 사용해서 받는다.
		response.getWriter().write(getJSON(name)); // 응답
	}
	...
	private String getJSON(String name) {
		...
		// ajaxboard 테이블에서 index.jsp에서 입력한 문자열이 name 필드에 포함된 데이터를 얻어온다.
