...
//	bean -> 자바로 처리하는 최소한의 작업 단위로 데잍러르 기억하는 필드(멤버 변수)와 필드에 저장된
//	데이터를 입출력하기 getter & setter 메소드로만 구성된 클래스
public class MemberVO {
	
//	필드 선언
//	VO 클래스의 필드 이름은 VO 클래스에 저장될 데이터가 입력되는 form을 구성하는 요소들의 
//	name 속성에 	지정한 속성값과 데이터베이스 테이블을 구성하는 필드 이름과 반드시 같은 이름으로 만들어야 한다.
	private String id;
	private String name;
